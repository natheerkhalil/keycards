<template>
  <div style="margin-top: 25px;" class="__b _flex _fd-co _cc">
    <router-link style="margin-bottom: 20px; " v-for="v in folders" class="__nun" to="folder/1">
      <div class="folder __po __hv-6 __hv __13 __w _flex __mlauto __mauto _fd-co __padsm __bo-err-7 __bdxs __bod">
        <div class="__b _flex _jc-be _ai-ce _m-sm-fd-co _m-sm-cc">
          <p @click="preventDefault" style="white-space: wrap; max-width: 90%; outline: none; " :id="`ftitle_${v.id}`"
            :contenteditable="v.editing"
            :class="[v.editing ? ['__un', '__bo-err-1', '__bod'] : '', '__txt-4', '__tmd', '__padxs']">{{ v.name }}</p>
          <p class="__txt-grey-4">{{ v.cards.total }} cards</p>
        </div>
        <br>
        <div
        ...
        <div class="__b _flex _jc-en _ai-ce _fd-ro">
          <svg :id="`share_${v.id}`" @click="share" class="__po __hfi-4" width="24" height="24"
          ...
          <svg :id="`destroy_${v.id}`" @click="destroy" class="__po __hfi-4" width="24" height="24" clip-rule="evenodd"
          ...
          <svg v-if="!v.editing" :id="`edit_${v.id}`" @click="edit" class="__po __hfi-4" width="24" height="24"
            xmlns="http://www.w3.org/2000/svg" fill-rule="evenodd" clip-rule="evenodd">
            <path
